...
namespace Server.Engines.BulkOrders
{
	...
	{
		#region Constructors
		private static readonly ConstructCallback Spade = new ConstructCallback( CreateSturdyShovel );
		private static readonly ConstructCallback Pickaxe = new ConstructCallback( CreateSturdyPickaxe );
		...
		private static readonly ConstructCallback DwarvenHammer = new ConstructCallback( CreateDwarvenHammer );

		private static Item CreateSturdyShovel( int type )
		{
			Item spade = new Spade();
			BaseAxe shovel = (BaseAxe)spade;
			spade.Name = "sturdy shovel";
			shovel.UsesRemaining = shovel.UsesRemaining * 3;
			return spade;
		}

		private static Item CreateSturdyPickaxe( int type )
		{
			Item chop = new Pickaxe();
			BaseAxe axe = (BaseAxe)chop;
			chop.Name = "sturdy pickaxe";
			axe.UsesRemaining = axe.UsesRemaining * 3;
			return chop;
		}
		...
		private static Item CreateRunicHammer( int type )
		{
			if ( type >= 1 && type <= 8 )
			{
				RunicHammer tool = new RunicHammer();
				tool.Resource = CraftResource.DullCopper;

				if ( type == 1 )
					tool.Resource = CraftResource.DullCopper;
				else if ( type == 2 )
					tool.Resource = CraftResource.ShadowIron;
				else if ( type == 3 )
					tool.Resource = CraftResource.Copper;
				else if ( type == 4 )
					tool.Resource = CraftResource.Bronze;
				else if ( type == 5 )
					tool.Resource = CraftResource.Gold;
				else if ( type == 6 )
					tool.Resource = CraftResource.Agapite;
				else if ( type == 7 )
					tool.Resource = CraftResource.Verite;
				else if ( type == 8 )
					tool.Resource = CraftResource.Valorite;
		...
		private static Item CreateDwarvenHammer( int type )
		{
			if ( type == 10 || type == 15 || type == 30 || type == 60 )
			{
				Item hammer = new SmithHammer();
				hammer.Resource = CraftResource.Dwarven;
				((BaseTool)hammer).UsesRemaining += type;
				return hammer;
			}
		...
		private RewardType[] m_Types = new RewardType[]
			{
				...
				new RewardType( 350, typeof( Broadsword ), typeof( Cutlass ), typeof( Katana ), typeof( Longsword ), typeof( Scimitar ), typeof( Claymore ), typeof( VikingSword ) ),
		...
		public SmithRewardCalculator()
		{
			Groups = new RewardGroup[]
				{
					new RewardGroup(    0, new RewardItem( 1, Spade ) ),
					new RewardGroup(   25, new RewardItem( 1, Pickaxe ) ),
					new RewardGroup(   50, new RewardItem( 45, Spade ), new RewardItem( 45, Pickaxe ), new RewardItem( 10, MiningGloves, 1 ) ),
					...
					new RewardGroup(  750, new RewardItem( 1, DwarvenHammer, 10 ) ),
					new RewardGroup(  800, new RewardItem( 1, PowerScroll, 15 ) ),
					new RewardGroup(  850, new RewardItem( 1, DwarvenHammer, 15 ) ),
					new RewardGroup(  900, new RewardItem( 1, PowerScroll, 20 ) ),
					new RewardGroup(  950, new RewardItem( 1, RunicHammer, 5 ) ),
					new RewardGroup( 1000, new RewardItem( 1, DwarvenHammer, 30 ) ),
					new RewardGroup( 1050, new RewardItem( 1, RunicHammer, 6 ) ),
					new RewardGroup( 1100, new RewardItem( 1, DwarvenHammer, 60 ) ),
	...
	public sealed class TailorRewardCalculator : RewardCalculator
	{
		...
		private static Item CreateCloth( int type )
		{
			if ( type >= 0 && type < m_ClothHues.Length )
			{
				...
				Item cloth = ResourceMods.GetRandomItem( item, null );
				return cloth;
			}
		...
		private static Item CreateRunicKit( int type )
		{
			if ( type >= 1 && type <= 3 )
			{
				RunicLeatherKit tool = new RunicLeatherKit();
				tool.Resource = CraftResource.SpinedLeather;

				if ( type == 1 )
					tool.Resource = CraftResource.SpinedLeather;
				else if ( type == 2 )
					tool.Resource = CraftResource.HornedLeather;
				else if ( type == 3 )
					tool.Resource = CraftResource.BarbedLeather;
